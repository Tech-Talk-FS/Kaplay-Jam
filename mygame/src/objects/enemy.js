	...
	add(){
		this.onStateEnter('idle', async ()=>{
			...
			if(this.path){
				...
				this.nextPos = vec2((0.5-Math.random())*s, (0.5-Math.random())*s).unit();
			}
		...
		this.onStateUpdate('persue', async () => {
			...
			if(DM.player.pos.dist(this.pos) > this.patrolRange) this.enterState('idle');
			if(this.hitbox.getCollisions().find(c=>c.target.tags.includes('player'))) this.enterState("attack"); 
			this.go(v);
		});
